...
def run_test_integers():
    ...
    # Test 2:
    expected = [473, 82, 3457, 9, -90, 46, 21, 555, 676, 308]
    answer = integers([(),
                       ('hello world', 'i', 'love'),
                       [473, 82, 'whatsup', 3457, 9, -90, [-89], 46],
                       [],
                       ['lets goooooo', 21, [1234], 555, [676], 676],
                       ['WHAT DID YOU SAY!?!?!'],
                       [[505], 308],
                       ])
    print('Expected is:', expected)
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'IMAFANOFDRMUTCHLER'
    answer = big_letters([(153, 346, 855),
                          'I love MAth?',
                          'For All NOs For yes', 'Do it foR the Meme',
                          ('TiMe AnD tImE AgAiN', 'BeLiEvEr', 308),
                          [],
                          ['POINT'],
                          'whatever it takes',
                          ['LESS'],
                          'Unless The CHime is made of Light!',
                          ['NESS'],
                          'take her to the emERgency room pronto!'
                          ])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
        ...
        for j in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k]) is str:
                if sequence_of_sequences[k][j].isupper():
                    new_str += sequence_of_sequences[k][j]
    return new_str
